...
{
  public:
    WaypointSaver(const std::string &waypoints_file) : waypoints_file_(waypoints_file), saved_waypoints_(false)
    {
        ros::NodeHandle n;
    ...
    void waypointsCallback(turtlebot3_momonga::WaypointArray waypoints)
    {
        ...
        for (unsigned int i = 0; i < size; i++)
        {
            ...
            savefile << waypoints.waypoints[i].pose.position.x << ","
                     ...
                     << waypoints.waypoints[i].waypoint_type << ","
